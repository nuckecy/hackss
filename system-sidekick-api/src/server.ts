...
const componentDataPath = join(__dirname, "..", "data", "sds-components.json");
const componentData = JSON.parse(readFileSync(componentDataPath, "utf-8"));
...
// Build a flat map of component names to their keys for detectComponentAction
...
function buildComponentKeyMap(
  data: typeof componentData
): Map<string, ComponentEntry> {
  const map = new Map<string, ComponentEntry>();

  // Primitives
  if (data.primitives) {
    for (const [name, comp] of Object.entries(data.primitives) as [
      string,
      any,
    ][]) {
      ...
      if (comp.relatedComponents) {
        for (const [relName, relComp] of Object.entries(
      ...
      if (comp.subcomponents && typeof comp.subcomponents === "object" && !Array.isArray(comp.subcomponents)) {
        for (const [subName, subComp] of Object.entries(
          comp.subcomponents
        ) as [string, any][]) {
      ...
      if (comp.variants && typeof comp.variants === "object") {
        for (const [varName, varComp] of Object.entries(comp.variants) as [
          string,
          any,
        ][]) {
  ...
  if (data.compositions) {
    for (const category of Object.values(data.compositions) as Record<
      string,
      any
    >[]) {
...
function detectComponentAction(text: string): ComponentAction | null {
  // Sort by name length descending so longer names match first (e.g., "ButtonDanger" before "Button")
  const sortedEntries = Array.from(componentKeyMap.entries()).sort(
    (a, b) => b[0].length - a[0].length
  );

  for (const [name, entry] of sortedEntries) {
    // Skip placeholder keys
    if (entry.componentKey === "YOUR_KEY_HERE") continue;
...
function detectVariant(text: string, componentName: string): string | null {
  ...
  const comp = Object.entries(componentData.primitives || {}).find(
